...
import {
    ...
    UpdateOptions,
    DeleteOptions
} from '../types/Gsoc2Client';
import { SecretService } from '../services';
import { ISecretBundle } from '../types/models';

export async function getSecretHelper(instance: Gsoc2Client, secretName: string, options: GetOptions): Promise<ISecretBundle> {
...
export async function getAllSecretsHelper(instance: Gsoc2Client, options: GetAllOptions): Promise<ISecretBundle[]> {
    try {
        if (!instance.clientConfig) throw Error('Failed to find client config');
